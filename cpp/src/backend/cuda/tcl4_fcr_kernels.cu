...
__device__ __forceinline__ cuFloatComplex cf_scale(cuFloatComplex a, float s) {
...
__global__ void kernel_scale_f32(cuFloatComplex* data, std::size_t n, float scale) {
    const std::size_t i = static_cast<std::size_t>(blockIdx.x) * blockDim.x + threadIdx.x;
    if (i < n) data[i] = cf_scale(data[i], scale);
}
...
__global__ void kernel_pointwise_mul_f32(cuFloatComplex* inout,
                                         const cuFloatComplex* other,
                                         std::size_t n)
{
    const std::size_t i = static_cast<std::size_t>(blockIdx.x) * blockDim.x + threadIdx.x;
    if (i < n) inout[i] = cf_mul(inout[i], other[i]);
}
...
__global__ void kernel_axpby_f32(const cuFloatComplex* x,
...
{
    const std::size_t i = static_cast<std::size_t>(blockIdx.x) * blockDim.x + threadIdx.x;
    if (i < n) out[i] = cf_add(cf_mul(a, x[i]), cf_mul(b, y[i]));
}
...
__global__ void kernel_pack_conv_F_f32(cuFloatComplex* A,
                                       cuFloatComplex* B,
                                       const cuFloatComplex* gamma,
                                       const float* omegas,
                                       const int* mirror,
                                       int i,
                                       int j,
                                       ...
                                       float dt)
{
    ...
    extern __shared__ cuFloatComplex smem_f32[];
    cuFloatComplex* g1_sh = smem_f32;
    cuFloatComplex* g2_sh = smem_f32 + blockDim.x;
    if (threadIdx.y == 0) {
        if (valid_g) {
            const std::size_t t_idx = t;
            g1_sh[threadIdx.x] = gamma[t_idx + Nt * static_cast<std::size_t>(i)];
            ...
            g1_sh[threadIdx.x] = make_cuFloatComplex(0.0f, 0.0f);
            g2_sh[threadIdx.x] = make_cuFloatComplex(0.0f, 0.0f);
        }
    }
    __syncthreads();
    if (!valid_t || !valid_b) return;
    ...
    const std::size_t base = b * Nfft;
    if (!valid_g) {
        A[base + t] = make_cuFloatComplex(0.0f, 0.0f);
        B[base + t] = make_cuFloatComplex(0.0f, 0.0f);
        return;
    }

    const int k = k0 + static_cast<int>(b);
    const float omega = omegas[i] + omegas[j] + omegas[k];
    const float theta = omega * dt * static_cast<float>(t);
    float s = 0.0f, c = 1.0f;
    sincosf(theta, &s, &c);
    const cuFloatComplex phase_minus = make_cuFloatComplex(c, -s);

    const cuFloatComplex g1 = g1_sh[threadIdx.x];
    const cuFloatComplex g2m = g2_sh[threadIdx.x];

    A[base + t] = cf_mul(g1, phase_minus);
    B[base + t] = g2m;
}
...
__global__ void kernel_pack_conv_C_f32(cuFloatComplex* A,
                                       cuFloatComplex* B,
                                       const cuFloatComplex* gamma,
                                       const float* omegas,
                                       int i,
                                       ...
                                       float dt)
{
    ...
    extern __shared__ cuFloatComplex smem_f32[];
    cuFloatComplex* g1_sh = smem_f32;
    cuFloatComplex* g2c_sh = smem_f32 + blockDim.x;
    if (threadIdx.y == 0) {
        if (valid_g) {
            const std::size_t t_idx = t;
            const cuFloatComplex g2 = gamma[t_idx + Nt * static_cast<std::size_t>(j)];
            g1_sh[threadIdx.x] = gamma[t_idx + Nt * static_cast<std::size_t>(i)];
            g2c_sh[threadIdx.x] = cf_conj(g2);
        } else {
            g1_sh[threadIdx.x] = make_cuFloatComplex(0.0f, 0.0f);
            g2c_sh[threadIdx.x] = make_cuFloatComplex(0.0f, 0.0f);
        }
    }
    __syncthreads();
    if (!valid_t || !valid_b) return;
    ...
    const cuFloatComplex g1 = g1_sh[threadIdx.x];
    const cuFloatComplex g2c = g2c_sh[threadIdx.x];
...
__global__ void kernel_pack_prefix_F_f32(cuFloatComplex* out, // [Nt, batch]
                                         const cuFloatComplex* gamma,
                                         const float* omegas,
                                         const int* mirror,
                                         int i,
                                         ...
                                         float dt)
{
    ...
    extern __shared__ cuFloatComplex smem_f32[];
    cuFloatComplex* g2m_sh = smem_f32;
    if (threadIdx.y == 0) {
        if (valid_t) {
            int jm = mirror[j];
            if (jm < 0) jm = j;
            ...
            g2m_sh[threadIdx.x] = make_cuFloatComplex(0.0f, 0.0f);
        }
    }
    __syncthreads();
    if (!valid_t || !valid_b) return;
    ...
    float s = 0.0f, c = 1.0f;
    sincosf(theta, &s, &c);
    const cuFloatComplex phase_plus = make_cuFloatComplex(c, s);

    const cuFloatComplex g2m = g2m_sh[threadIdx.x];
    out[t + Nt * b] = cf_scale(cf_mul(g2m, phase_plus), dt);
}
...
__global__ void kernel_pack_prefix_C_f32(cuFloatComplex* out, // [Nt, batch]
                                         const cuFloatComplex* gamma,
                                         const float* omegas,
                                         int i,
                                         int j,
                                         ...
                                         float dt)
{
    ...
    extern __shared__ cuFloatComplex smem_f32[];
    cuFloatComplex* g2c_sh = smem_f32;
    if (threadIdx.y == 0) {
        if (valid_t) {
            const cuFloatComplex g2 = gamma[t + Nt * static_cast<std::size_t>(j)];
            g2c_sh[threadIdx.x] = cf_conj(g2);
        } else {
            g2c_sh[threadIdx.x] = make_cuFloatComplex(0.0f, 0.0f);
        }
    ...
    const cuFloatComplex g2c = g2c_sh[threadIdx.x];
    out[t + Nt * b] = cf_scale(cf_mul(g2c, phase_plus), dt);
}
...
__global__ void kernel_assemble_FC_f32(cuFloatComplex* out, // [Nt, batch] prefix in, result out
                                       const cuFloatComplex* conv, // [Nfft, batch]
                                       const cuFloatComplex* gamma,
                                       const float* omegas,
                                       int i,
                                       int j,
                                       ...
                                       float dt)
{
    ...
    const cuFloatComplex g1 = g1_sh[threadIdx.x];
    const cuFloatComplex g1_phase = cf_mul(g1, phase_minus);
    const cuFloatComplex prefix = out[t + Nt * b];
    const cuFloatComplex term1 = cf_mul(g1_phase, prefix);
    const cuFloatComplex term2 = conv[t + Nfft * b];
    out[t + Nt * b] = make_cuFloatComplex(term1.x - term2.x, term1.y - term2.y);
}
...
__global__ void kernel_pack_R_integrands_f32(cuFloatComplex* out_g2, // [Nt, batch]
                                             cuFloatComplex* out_P,  // [Nt, batch]
                                             const cuFloatComplex* gamma,
                                             const float* omegas,
                                            int i,
                                            int j,
                                            ...
                                            float dt)
{
    ...
    const cuFloatComplex g1 = g1_sh[threadIdx.x];
    const cuFloatComplex g2 = g2_sh[threadIdx.x];

    const std::size_t idx = t + Nt * b;
    out_g2[idx] = cf_scale(cf_mul(g2, phase_minus), dt);
    out_P[idx] = cf_scale(cf_mul(cf_mul(g1, g2), phase_minus), dt);
}
...
__global__ void kernel_assemble_R_f32(cuFloatComplex* out_R, // [Nt, batch] prefix_g2 in, R out
                                      const cuFloatComplex* prefix_P, // [Nt, batch]
                                      const cuFloatComplex* gamma,
                                      int i,
                                      int batch,
                                      std::size_t Nt)
{
    ...
    const std::size_t idx = t + Nt * b;
    const cuFloatComplex g1 = g1_sh[threadIdx.x];
    const cuFloatComplex term1 = cf_mul(g1, out_R[idx]);
    const cuFloatComplex term2 = prefix_P[idx];
    out_R[idx] = make_cuFloatComplex(term1.x - term2.x, term1.y - term2.y);
}
} // namespace
...
void launch_scale_f32(cuFloatComplex* data,
                      std::size_t n,
                      float scale,
                      cudaStream_t stream)
{
    constexpr int block = 256;
    kernel_scale_f32<<<grid_1d(n, block), block, 0, stream>>>(data, n, scale);
    cuda_check(cudaGetLastError(), "kernel_scale_f32 launch");
}

void launch_pointwise_mul_f32(cuFloatComplex* inout,
                              const cuFloatComplex* other,
                              std::size_t n,
                              cudaStream_t stream)
{
    constexpr int block = 256;
    kernel_pointwise_mul_f32<<<grid_1d(n, block), block, 0, stream>>>(inout, other, n);
    cuda_check(cudaGetLastError(), "kernel_pointwise_mul_f32 launch");
}
...
{
    constexpr int block = 256;
    kernel_axpby_f32<<<grid_1d(n, block), block, 0, stream>>>(x, y, out, n, a, b);
    cuda_check(cudaGetLastError(), "kernel_axpby_f32 launch");
}
...
struct CaddF32 {
  __host__ __device__ cuFloatComplex operator()(cuFloatComplex a, cuFloatComplex b) const {
    return make_cuFloatComplex(a.x + b.x, a.y + b.y);
  }
};
...
void convolution_cufft_f32(cufftHandle plan,
                           cuFloatComplex* A,  // [batch * Nfft] in/out
                           cuFloatComplex* B,  // [batch * Nfft] in/out
                           std::size_t batch,
                           std::size_t Nfft,
                           float dt,
                           cudaStream_t stream)
{
    const std::size_t n = batch * Nfft;
    cufftSetStream(plan, stream);
    auto* Ac = reinterpret_cast<cufftComplex*>(A);
    auto* Bc = reinterpret_cast<cufftComplex*>(B);

    cufftExecC2C(plan, Ac, Ac, CUFFT_FORWARD);
    cufftExecC2C(plan, Bc, Bc, CUFFT_FORWARD);

    launch_pointwise_mul_f32(A, B, n, stream);
    cufftExecC2C(plan, Ac, Ac, CUFFT_INVERSE);
    launch_scale_f32(A, n, dt / static_cast<float>(Nfft), stream);
}
                                         ...
                                         cudaStream_t stream)
{
    ...
    if (B == 0 || Nt == 0) return;
    if (!inputs.gamma || !inputs.omegas || !inputs.mirror) throw std::runtime_error("compute_fcr_convolution_batched_f32: null inputs");
    if (!batch.F || !batch.C || !batch.R) throw std::runtime_error("compute_fcr_convolution_batched_f32: null outputs");
    if (Nfft < 2 * Nt - 1) throw std::runtime_error("compute_fcr_convolution_batched_f32: Nfft must be >= 2*Nt-1");
    if (!(inputs.dt > 0.0f)) throw std::runtime_error("compute_fcr_convolution_batched_f32: dt must be > 0");

    if (batch.i < 0 || batch.j < 0 || batch.k0 < 0) throw std::runtime_error("compute_fcr_convolution_batched_f32: negative index");
    if (static_cast<std::size_t>(batch.i) >= nf || static_cast<std::size_t>(batch.j) >= nf) {
        throw std::runtime_error("compute_fcr_convolution_batched_f32: i/j out of range");
    }
    if (static_cast<std::size_t>(batch.k0) + B > nf) {
        throw std::runtime_error("compute_fcr_convolution_batched_f32: k0+batch out of range");
    }
    if (Nt > static_cast<std::size_t>(std::numeric_limits<int>::max())) {
        throw std::runtime_error("compute_fcr_convolution_batched_f32: Nt too large for CUB DeviceScan");
    }
    if (B > static_cast<std::size_t>(std::numeric_limits<int>::max()) ||
        Nfft > static_cast<std::size_t>(std::numeric_limits<int>::max())) {
        throw std::runtime_error("compute_fcr_convolution_batched_f32: batch/Nfft too large for cuFFT plan");
    }
    ...
    const std::size_t scratch_elems = B * Nfft;
    const std::size_t scratch_bytes = scratch_elems * sizeof(cuFloatComplex);
    if (!ws.A) cuda_check(cudaMalloc(reinterpret_cast<void**>(&ws.A), scratch_bytes), "cudaMalloc(ws.A)");
    if (!ws.B) cuda_check(cudaMalloc(reinterpret_cast<void**>(&ws.B), scratch_bytes), "cudaMalloc(ws.B)");
    ...
    // ---- ensure scan temp (for length Nt scans) ----
    {
        ...
        cub::DeviceScan::InclusiveScan(nullptr, need,
                                       ws.A, ws.A,
                                       CaddF32{}, static_cast<int>(Nt), stream);
        if (need > ws.scan_tmp_bytes) {
            if (ws.scan_tmp) cuda_check(cudaFree(ws.scan_tmp), "cudaFree(ws.scan_tmp)");
            cuda_check(cudaMalloc(&ws.scan_tmp, need), "cudaMalloc(ws.scan_tmp)");
            ws.scan_tmp_bytes = need;
    ...
    const FcrBlockConfig cfg64 = get_fcr_block_config();
    const int block_t = cfg64.block_t;
    int block_b = cfg64.block_b;
    if (block_b > static_cast<int>(B)) block_b = static_cast<int>(B);
    if (block_b < 1) block_b = 1;

    const std::size_t smem1 = static_cast<std::size_t>(block_t) * sizeof(cuFloatComplex);
    const std::size_t smem2 = static_cast<std::size_t>(block_t) * 2 * sizeof(cuFloatComplex);
    ...
    const bool want_profile = fcr_profile_enabled();
    static std::atomic<int> profile_once_f32{0};
    const bool profile_this = want_profile && (profile_once_f32.fetch_add(1) == 0);
    cudaEvent_t ev_start = nullptr;
    cudaEvent_t ev_f = nullptr;
    cudaEvent_t ev_c = nullptr;
    cudaEvent_t ev_r = nullptr;
    ...
    // ---- F: g2m (mirror[j]) ----
    kernel_pack_conv_F_f32<<<grid_conv, block, smem2, stream>>>(ws.A, ws.B,
                                                                inputs.gamma, inputs.omegas, inputs.mirror,
                                                                batch.i, batch.j, batch.k0, static_cast<int>(B),
                                                                Nt, Nfft, inputs.dt);
    cuda_check(cudaGetLastError(), "kernel_pack_conv_F_f32 launch");
    convolution_cufft_f32(ws.plan, ws.A, ws.B, B, Nfft, inputs.dt, stream);

    kernel_pack_prefix_F_f32<<<grid_time, block, smem1, stream>>>(ws.B,
                                                                  inputs.gamma, inputs.omegas, inputs.mirror,
                                                                  batch.i, batch.j, batch.k0, static_cast<int>(B),
                                                                  Nt, inputs.dt);
    cuda_check(cudaGetLastError(), "kernel_pack_prefix_F_f32 launch");
    for (std::size_t b = 0; b < B; ++b) {
        const cuFloatComplex* in = ws.B + b * Nt;
        cuFloatComplex* out = batch.F + b * Nt;
        cub::DeviceScan::InclusiveScan(ws.scan_tmp, ws.scan_tmp_bytes,
                                       in, out,
                                       CaddF32{}, static_cast<int>(Nt), stream);
    }
    kernel_assemble_FC_f32<<<grid_time, block, smem1, stream>>>(batch.F, ws.A,
                                                                inputs.gamma, inputs.omegas,
                                                                batch.i, batch.j, batch.k0, static_cast<int>(B),
                                                                Nt, Nfft, inputs.dt);
    cuda_check(cudaGetLastError(), "kernel_assemble_FC_f32(F) launch");
    if (profile_this) cuda_check(cudaEventRecord(ev_f, stream), "cudaEventRecord(F)");

    // ---- C: conj(g2) ----
    kernel_pack_conv_C_f32<<<grid_conv, block, smem2, stream>>>(ws.A, ws.B,
                                                                inputs.gamma, inputs.omegas,
                                                                batch.i, batch.j, batch.k0, static_cast<int>(B),
                                                                Nt, Nfft, inputs.dt);
    cuda_check(cudaGetLastError(), "kernel_pack_conv_C_f32 launch");
    convolution_cufft_f32(ws.plan, ws.A, ws.B, B, Nfft, inputs.dt, stream);

    kernel_pack_prefix_C_f32<<<grid_time, block, smem1, stream>>>(ws.B,
                                                                  inputs.gamma, inputs.omegas,
                                                                  batch.i, batch.j, batch.k0, static_cast<int>(B),
                                                                  Nt, inputs.dt);
    cuda_check(cudaGetLastError(), "kernel_pack_prefix_C_f32 launch");
    for (std::size_t b = 0; b < B; ++b) {
        const cuFloatComplex* in = ws.B + b * Nt;
        cuFloatComplex* out = batch.C + b * Nt;
        cub::DeviceScan::InclusiveScan(ws.scan_tmp, ws.scan_tmp_bytes,
                                       in, out,
                                       CaddF32{}, static_cast<int>(Nt), stream);
    }
    kernel_assemble_FC_f32<<<grid_time, block, smem1, stream>>>(batch.C, ws.A,
                                                                inputs.gamma, inputs.omegas,
                                                                batch.i, batch.j, batch.k0, static_cast<int>(B),
                                                                Nt, Nfft, inputs.dt);
    cuda_check(cudaGetLastError(), "kernel_assemble_FC_f32(C) launch");
    if (profile_this) cuda_check(cudaEventRecord(ev_c, stream), "cudaEventRecord(C)");

    // ---- R: prefix(g2*e^{-iwt}) and prefix((g1*g2)*e^{-iwt}) ----
    kernel_pack_R_integrands_f32<<<grid_time, block, smem2, stream>>>(ws.A, ws.B,
                                                                      inputs.gamma, inputs.omegas,
                                                                      batch.i, batch.j, batch.k0, static_cast<int>(B),
                                                                      Nt, inputs.dt);
    cuda_check(cudaGetLastError(), "kernel_pack_R_integrands_f32 launch");
    for (std::size_t b = 0; b < B; ++b) {
        const cuFloatComplex* in = ws.A + b * Nt;
        cuFloatComplex* out = batch.R + b * Nt;
        cub::DeviceScan::InclusiveScan(ws.scan_tmp, ws.scan_tmp_bytes,
                                       in, out,
                                       CaddF32{}, static_cast<int>(Nt), stream);
    }
    for (std::size_t b = 0; b < B; ++b) {
        const cuFloatComplex* in = ws.B + b * Nt;
        cuFloatComplex* out = ws.A + b * Nt; // reuse ws.A for prefix_P
        cub::DeviceScan::InclusiveScan(ws.scan_tmp, ws.scan_tmp_bytes,
                                       in, out,
                                       CaddF32{}, static_cast<int>(Nt), stream);
    }
    kernel_assemble_R_f32<<<grid_time, block, smem1, stream>>>(batch.R, ws.A,
                                                               inputs.gamma,
                                                               batch.i, static_cast<int>(B), Nt);
    cuda_check(cudaGetLastError(), "kernel_assemble_R_f32 launch");
    if (profile_this) {
        cuda_check(cudaEventRecord(ev_r, stream), "cudaEventRecord(R)");
        cuda_check(cudaEventSynchronize(ev_r), "cudaEventSynchronize(R)");
        float ms_f = 0.0f;
        ...
        cuda_check(cudaEventElapsedTime(&ms_r, ev_c, ev_r), "cudaEventElapsedTime(R)");
        std::fprintf(stderr,
                     "cuda_fcr f32 profile: block=%dx%d B=%zu Nt=%zu Nfft=%zu ms_F=%.3f ms_C=%.3f ms_R=%.3f\n",
                     block_t, block_b, B, Nt, Nfft, ms_f, ms_c, ms_r);
        cuda_check(cudaEventDestroy(ev_start), "cudaEventDestroy(start)");
        cuda_check(cudaEventDestroy(ev_f), "cudaEventDestroy(F)");
        cuda_check(cudaEventDestroy(ev_c), "cudaEventDestroy(C)");
