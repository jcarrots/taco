/*
--------------------------------------------------------------------------------
  correlation.cpp — Bath correlation function helpers and FFT backends
--------------------------------------------------------------------------------

  Overview
  - Builds a time‑domain bath correlation C(t) from a spectral density J(ω)
    using a discrete Fourier transform.
  - Provides small, dependency‑light FFT utilities with two backends:
      1) pocketfft (BSD, header‑only) when available
      2) an in‑house radix‑2 Cooley–Tukey FFT for power‑of‑two sizes
  - Includes a minimal “plan” wrapper to reuse scratch for repeated transforms.

  Key API (namespace bcf)
  - using cd = std::complex<double>
  - next_pow2(n): round up to the next power of two
  - fft_inplace_builtin_dir(a, forward): directional in‑place radix‑2 FFT
  - fft_inplace(a): forward FFT (pocketfft if present, else in‑house)
  - ifft_inplace(a): inverse FFT with 1/N scaling
  - struct FFTPlan { exec_forward(a); exec_inverse(a); } — reuses scratch
  - template <JCallable>
    bcf_fft_fun(N, dt, J, beta, t, C): build C(t) from J(ω) at temperature β

  Mathematical conventions
  - We construct a discrete spectrum J(ω; β) from J(ω≥0) using detailed balance
        J(−ω) = e^{−βω} J(+ω)
    with (π/2) factors chosen so that the correlation function is
        C(τ) = (1/2π) ∫_{−∞}^{∞} dω J(ω;β) e^{−iωτ}
    is approximated by our discrete inverse FFT followed by a scale `Δω/π`.
  - Grid relationships (uniform FFT grid):
        Δω = 2π / (Nfft · dt),  τ_n = n · dt,  n=0..Nfft−1
    bcf_fft_fun returns only the first Nt=N+1 non‑negative samples (0..N).
  - DC and Nyquist bins are handled with half‑weights for stability.

  Temperature handling
  - T=0 (β→∞): only positive frequencies contribute: J(+ω;0) = (π/2) J(ω)
  - Finite T: uses the KMS/detailed‑balance relation via S(−ω)=e^{−βω} S(+ω)
    and a stable small‑ω limit for the DC term.

  Backends and configuration
  - If CMake defines TACO_USE_POCKETFFT and a pocketfft header is found
    (<pocketfft/pocketfft.h> or "pocketfft_hdronly.h"), pocketfft is used.
  - Otherwise the in‑house radix‑2 kernel is used automatically.

  Limitations and tips
  - The in‑house FFT requires power‑of‑two sizes. bcf_fft_fun rounds up to
    the next power of two internally for symmetry grids.
  - Choose Nfft and dt so that your time horizon (N·dt) and frequency band are
    large enough to avoid wrap‑around/aliasing. Zero‑padding in frequency (via
    larger Nfft) reduces ringing in time.
  - For strongly peaked spectra, consider windowing (not implemented here) or
    increase Nfft to reduce spectral leakage.

  Example
    // Ohmic‑Drude J(ω) = 2 λ γ ω / (ω^2 + γ^2)
    auto J = [&](double w){ return (w<=0)?0.0 : 2.0*lambda*gamma*w/(w*w + gamma*gamma); };
    std::vector<double> t; std::vector<bcf::cd> C;
    bcf::bcf_fft_fun(8191, 1e-2, J, 5.0, t, C);
