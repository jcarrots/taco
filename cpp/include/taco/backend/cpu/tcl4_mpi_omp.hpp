...
// - OpenMP: parallelizes within each rank over its assigned time indices (controlled by `exec.threads`).
//
// Collective over `comm`: all ranks in `comm` must call with the same inputs.
//
